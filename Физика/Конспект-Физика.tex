...
\begin{document}
	...
	\end{definition}
	\begin{definition}
		$|\varepsilon| = \dfrac{|\varDelta l|}{l_0}$.
	\end{definition}
	...
	\end{definition}
	\begin{corollary}[Основное уравнение МКТ в энергетической форме]
		$p = \dfrac{2}{3} n \overline{E}$.
	\end{corollary}
	\subsection{Температура}
	\begin{definition}[Формула связи средней кинетической энергии молекулы с температурой]
		$\overline{E} = \dfrac{3}{2}kT$, $k = 1.38 \cdot 10^{-23} \dfrac{\text{Дж}}{\text{К}}$.
	\end{definition}
	\begin{definition}[Формула среднеквадратичной скорости молекул]
	...
	\begin{definition}[Приближение Ван дер Вальса]
	...
	\end{definition}
	\begin{definition}[Перенасыщенный пар]
	...
	\begin{definition}[Абсолютная плотность воздуха]
	...
	\begin{definition}[Внутренняя энергия, $U$ Дж]
		Суммарная кинетическая энергия молекул $+$ потенциальная энергия их взаимодействия. В идеальном газе только суммарная кинетическая энергия. $U = N \cdot \overline{E} = N_A \nu \cdot \dfrac{3}{2} KT = \dfrac{3}{2} \nu RT$~--- внутренняя энергия одноатомного идеального газа.
	\end{definition}
	\begin{definition}[Степень свободы газа, $i$]
		Степени свободы газов~--- характеристики движения механической системы. Число степеней свободы определяет минимальное количество независимых переменных (обобщённых координат), необходимых для полного описания состояния механической системы.
		\begin{itemize}
			\item Одноатомный газ: $i = 3$.
			\item Двухатомный газ: $i = 5$.
			\item Многоатомный газ: $i = 6$.
		\end{itemize}
		Формула выглядит вот так: $U = \dfrac{i}{2} \nu RT$.
	\end{definition}
	\begin{definition}[Работа газа]
		$A = p \cdot \varDelta V$. Численно равна площади под графиком в $p(\varDelta V)$ координатах.
	\end{definition}
	\subsection{Теплопередача}
	\begin{definition}[Теплопередача]
		Передача энергии без совершения работы.
	\end{definition}
	Три способа теплопередачи:
	\begin{itemize}
		\item Конвекция~--- перемещение и перемешивание теплых слоев жидкостей и газов.
		\item Теплопроводность~--- проводимость энергии от более нагретой части тела к менее нагретой путем хаотичного колебания частиц тела.
		\item Излучение~--- поток фатов/электромагнитная волна. Также происходит в вакууме, в отличии от других способов.
	\end{itemize}
	\subsection{Первое начало термодинамики}
	$U_1 + Q + A_{\text{над телом}} = U_2 \Leftrightarrow Q = -A_{\text{над телом}} + \varDelta U \Leftrightarrow Q = A_{\text{тела}} + \varDelta U$.
	\begin{xltabular}{\textwidth}{|X|X|X|}
		\hline
		Процесс & Первое начало & Комментарий\\
		\hline
		Изохорный, $V = const$ & $Q = \varDelta U = \dfrac{i}{2} \nu R \varDelta T$ & $A_{\text{тела}} = 0$; $\uparrow T \rightarrow \uparrow U \Rightarrow \varDelta U > 0$\\
		\hline
		Изотермический, $T = const$ & $Q = A_{\text{тела}}$ & $T = const$, $T_1 = T_2 \Rightarrow \varDelta T = 0 \Rightarrow \varDelta U = 0$. $\uparrow V \Rightarrow A_{\text{тела}} > 0 \Rightarrow Q > 0$\\
		\hline
		Изобарный, $p = const$ & $Q = A_{\text{тела}} + \varDelta U = p \varDelta V + \dfrac{i}{2} \nu R \varDelta T = p \varDelta V + \dfrac{i}{2} p \varDelta V = (1 + \dfrac{i}{2})p \varDelta V = (1 + \dfrac{i}{2}) \nu R \varDelta T$ & $T \uparrow$, $\varDelta U = \dfrac{i}{2} \nu RT = \dfrac{i}{2} \varDelta (pV) = \dfrac{i}{2} p \varDelta V$\\
		\hline
		Адиабатный & $0 = A_{\text{тела}} + \varDelta U$ & Либо в термосе, либо очень быстро. $Q = 0$. $A_{\text{тела}} > 0$, $V \uparrow$; $\varDelta U < 0$, $T \downarrow$\\
		\hline
	\end{xltabular}
	\subsection{Теплоемкость}
	\begin{definition}[Теплоемкость]
		$c = \dfrac{Q}{m \varDelta T} = \dfrac{A + \varDelta U}{m \varDelta T}$
	\end{definition}
	\begin{enumerate}
		\item Изохорный процесс.\\
		$V = const \Rightarrow A = 0$\\
		$c = \dfrac{\varDelta U}{m \varDelta T} = \dfrac{\frac{i}{2} \nu R \varDelta T}{\nu M \varDelta T} = \dfrac{\frac{i}{2} R}{M}$\\
		$c_{M_V} = c \cdot M = \dfrac{iR}{2}$
		\item Изобарный процесс.\\
		$p = const$\\
		$c = \dfrac{\varDelta U + A}{m \varDelta T} = \dfrac{\frac{i}{2} R \varDelta T \nu + p \varDelta V}{m \varDelta T} = \dfrac{\frac{i}{2} R \varDelta T \nu + \nu R \varDelta T}{m \varDelta T} = \left( \dfrac{i}{2} + 1 \right) \dfrac{R}{M}$\\
		$c_{M_p} = \left( \dfrac{i}{2} + 1 \right) R = c_{M_V} + R$~--- соотношение Майера.\\
		$\gamma = \dfrac{c_{M_p}}{c_{M_V}} = \dfrac{i + 2}{i}$~--- коэффициент Пуассона.
		\item Изотермический процесс.\\
		$c = \dfrac{Q}{m \varDelta T} = \dfrac{A}{m \varDelta T}$\\
		$T = const \Rightarrow \varDelta T = 0 \Rightarrow \varDelta U = 0 \Rightarrow c \rightarrow \infty$
		\item Адиабатный процесс.\\
		$Q = 0$\\
		$c = \dfrac{Q}{m \varDelta T} = 0$
	\end{enumerate}
	\subsection{Адиабатный процесс}
	\begin{definition}
		Адиабатный процесс~--- процесс, при котором не подводится тепло.
	\end{definition}
	% $Q = 0 \Rightarrow A_{\text{г}} = - \varDelta U = - \dfrac{i}{2} \nu R \varDelta T = c_{m_V} \nu (\dfrac{p_1V_1}{\nu R} - \dfrac{p_2V_2}{\nu R}) = c_{m_V} \dfrac{p_1V_1 - p_2V_2}{c_{m_p} - c_{m_V}} = \dfrac{p_1V_1 - p_2V_2}{\frac{c_{m_p}}{c_{m_V}} - 1}$
	\begin{statement}
		$pV^\gamma = const$.
	\end{statement}
	\subsection{Необратимость процессов}
	\begin{definition}
		Тепловая машина~--- устройство, которое превращает тепло в работу.
	\end{definition}
	\subsubsection{Второе начало термодинамики}
	\begin{definition}[Формулировка Клаузиуса]
		Невозможен процесс единственным результатом которого будет переход тепла от холодного тела к горячему.
	\end{definition}
	\begin{definition}[Формулировка Кельвина]
		Невозможен циклический процесс, единственным результатом которого будет полное преобразование тепла в работу.
	\end{definition}
	\subsection{Тепловые машины}
	\begin{definition}[КПД]
		$\eta = \dfrac{A}{Q_{\text{н}}} = \dfrac{Q_{\text{н}} - Q_{\text{х}}}{Q_{\text{н}}} = 1 - \dfrac{Q_{\text{х}}}{Q_{\text{н}}}$.
	\end{definition}
	\subsubsection{Теорема Карно}
	\begin{definition}
		$\eta = \dfrac{T_1 - T_2}{T_1}$.
	\end{definition}
	\subsubsection{Холодильный коэффициент}
	\begin{definition}
		$\xi = \dfrac{Q_{\text{х}}}{A} = \dfrac{T_2}{T_1 - T_2}$.
	\end{definition}
	\section{Поверхностное натяжение}
	\begin{definition}[Энергия поверхностного натяжения]
		$W_{\text{пов}}$, Дж.
	\end{definition}
	\begin{definition}[Коэффициент поверхностного натяжения]
		$\sigma = \dfrac{W_{\text{пов}}}{S}$, $\dfrac{\text{Дж}}{\text{м}^2}$.
	\end{definition}
	\begin{definition}[Сила поверхностного натяжения]
		$F = \sigma P$, $P$~--- периметр соприкосновения жидкости.
	\end{definition}
	\subsection{Капиллярные явления}
	\begin{definition}[Мениск]
		Искривленная, за счет поверхностных явлений, поверхность жидкости.
	\end{definition}
	\subsubsection{Виды смачиваний}
	\begin{enumerate}
		\item Полное смачивание (угол между жидкостью и поверхностью $0$\textdegree).
		\item Неполное (угол не $0$\textdegree).
	\end{enumerate}
	\begin{statement}
		$h = \dfrac{2 \sigma}{\rho g r}$.
	\end{statement}
	\begin{definition}[Избыточное давление Лапласа под искривленной поверхностью]
		$p = \dfrac{2 \sigma}{r}$.
	\end{definition}
	\section{Тепловое расширение}
	\begin{definition}
		Изотропность~--- сохранение свойств, в независимости от направления. Анизотропность~--- изменение свойств, в зависимости от направления.
	\end{definition}
	\begin{definition}
		$l = l_0 (1 + \alpha \varDelta t)$; $\alpha$~--- коэффициент линейного расширения, $[\alpha] = \dfrac{1}{\text{\textdegree} C}$.
	\end{definition}
	\begin{definition}
		$\beta$~--- коэффициент объемного расширения; $\beta = 3\alpha$.
	\end{definition}
	\begin{definition}
		$\rho = \dfrac{\rho_0}{1 + \beta \varDelta t} \approx \rho_0 (1 - \beta \varDelta t)$.
	\end{definition}
	\begin{definition}[Механическое напряжение]
		$\sigma = \frac{F}{S} = \varepsilon \cdot \frac{kl_0}{S} = E \cdot |\varepsilon|$. $[\sigma] = \frac{\text{Н}}{\text{м}^2} = \text{Па}$.
	\end{definition}
	...
	\subsubsection{Электростатика}
	
	\begin{definition}[Заряд, $q$ Кл]
		Отрицательно заряженное тело~--- избыток электронов, положительно~--- недостаток. Заряд электрона: $-1.6 \cdot 10^{-19} \, \text{Кл}$; протона аналогичный, но с плюсом.
	\end{definition}
	
	\begin{definition}[Проводники и диэлектрики]
		В проводниках в узлах кристалической решетки~--- ионы; в диэлектриках~--- атомы, без свободных электронов.
	\end{definition}
	
	\paragraph{Закон Кулона}
	
	Кулон открыл, что $F \sim q_1$, $F \sim q_2$, $F \sim \dfrac{1}{r^2}$. Позднее была получена формула:
	$$
	F = \dfrac{k |q_1| |q_2|}{\varepsilon r^2}, \, k = 9 \cdot 10^9 \, \dfrac{\text{Н} \cdot \text{м}^2}{\text{Кл}^2},
	$$
	...
	\begin{itemize}
		\item тела~--- точечные,
		\item тела не двигаются.
	\end{itemize}
	
	\paragraph{Принцип суперпозиции}
	
	Суть принципа суперпозиции не в том, что ``можно складывать вектора'', а в том, что наличие других сил не влияет на данную.
	
	\paragraph{Электрическое поле}
	
	\begin{definition}[Электрическое поле]
		Особая форма материи, которая окружает электрические заряды и оказывает силовое воздействие на другие заряды, притягивая или отталкивая их, являясь проявлением электромагнитного взаимодействия, описываемого через такие характеристики, как напряженность (векторная величина), потенциал и напряжение, а силовые линии помогают визуализировать его структуру.
	\end{definition}
	
	\begin{definition}[Напряженность, $E \, \dfrac{\text{В}}{\text{м}}$]
		$\vec{E} = \dfrac{\vec{F}}{q_0}$. Для точечного заряда напряженность равна $\dfrac{kQ}{\varepsilon r^2}$.
	\end{definition}
	
	\begin{definition}[Однородное поле]
		Поле, в любой точке которого одинаковы сила и направление этой силы.q
	\end{definition}
	
	\subsubsection{Теорема Гаусса}
	
	\begin{definition}[Поток вектора напряженности электрического поля, $\Phi_{E} \, \text{В} \cdot \text{м}$]
		$\varDelta \Phi_{E} = E \cdot \varDelta S \cdot \cos \theta$, где $\theta$~--- угол между $\vec{E}$ и $\vec{n}$.
	\end{definition}
	
	\begin{definition}[Телесный угол, $\Omega \, \text{стерадианы (ср)}$]
		$\Omega = \dfrac{S}{R^2}$.
	\end{definition}
	
	\begin{theorem}
		Поток вектора напряженности электрического поля для выбранного контура равен $\Phi_{E} = 4 \pi k \sum q_{\text{внутри}}$.
	\end{theorem}
	
	\subsubsection{Поля статичного распределения заряда}
	
	\begin{definition}[Поверхностная плотность заряда, $\sigma \, \dfrac{\text{Кл}}{\text{м}}$]
		$\sigma = \dfrac{q}{S}$.
	\end{definition}
	
	\begin{definition}[Напряженность электрического поля бесконечной равномерно заряженной плоскости]
		$E = 2 \pi k \sigma = \dfrac{\sigma}{2 \varepsilon_0}$.
	\end{definition}
	
	\subsubsection{Работа сил электрического поля}
	
	\begin{definition}[Работа, $A \, \text{Дж}$]
		$A = F l \cos \alpha = q E l \cos \alpha$.
	\end{definition}
	...
	\begin{definition}[Потенциал, $\varphi \, \text{В}$]
		$\varphi = \dfrac{W}{q}$.
	\end{definition}
	
	\begin{statement}[Связь между напряженностью однородного электрического поля и разностью потенциалов (напряжением)]
		$\underbrace{\varphi_1 - \varphi_2}_U = El$.
	\end{statement}
	
	\begin{statement}[Потенциал электрического поля точечного заряда]
		$\varphi = \dfrac{kQ}{r}$.
	\end{statement}
	
	\begin{definition}[Энергия взаимодействия, $W \, \text{Дж}$]
		$W = \dfrac{kQq}{r}$.
	\end{definition}
	
	\subsection{Конденсаторы}
	
	\begin{definition}[Емкость конденсатора, $c \, \text{Ф}$]
		$c = \dfrac{q}{U} = \dfrac{\varepsilon \varepsilon_0 S}{d}$; $\varepsilon_0 = \dfrac{1}{4 \pi k} = 8.85 \cdot 10^{-12} \dfrac{\text{Кл}^2}{\text{Н} \cdot \text{м}^2}$.
	\end{definition}
	
	\begin{definition}[Энергия конденсатора]
		$W = \dfrac{c U^2}{2}$.
	\end{definition}
	...
	$$
	\dfrac{1}{c_{\text{об}}} = \dfrac{1}{c_1} + \dfrac{1}{c_2} + \dots
	$$
	$$
	...
	\begin{definition}[Сила тока, $I \, \text{А}$]
		$I = \dfrac{q}{t}$.
	\end{definition}
	
	\begin{definition}[Плотность тока, $j \, \dfrac{\text{А}}{\text{м}^2}$]
		$j = \dfrac{I}{S}$.
	\end{definition}
	
	\begin{definition}[Электро-движущая сила, $\varepsilon \, \text{В}$]
		$\varepsilon = \dfrac{A_{\text{ст}}}{q}$. Работа сторонних сил имеет не электрический характер.
	\end{definition}
	
	\begin{definition}[Сопротивление резистора, $R \, \text{Ом}$]
		$R = \dfrac{\rho l}{S}$.
	\end{definition}
	
	\begin{definition}[Последовательное соединение резисторов]
		\[ I_{\text{об}} = I_{1} = I_{2} = \dots \]
		\[ U_{\text{об}} = U_{1} + U_{2} + \dots \]
		\[ R_{\text{об}} = R_{1} + R_{2} + \dots \]
	\end{definition}
	
	\begin{definition}[Параллельное соединение резисторов]
		\[ I_{\text{об}} = I_{1} + I_{2} + \dots \]
		\[ U_{\text{об}} = U_{1} = U_{2} = \dots \]
		\[ \frac{1}{R_{\text{об}}} = \frac{1}{R_{1}} + \frac{1}{R_{2}} + \dots \]
	\end{definition}
	
	\begin{definition}[Мощность тока, $P \, \text{Вт}$]
		$P = IU = I^2 R = \dfrac{U^2}{R}$.
	\end{definition}
	
	\begin{definition}[Теплота, выделяемая током, $Q \, \text{Дж}$]
		$Q = P \varDelta t$.
	\end{definition}
	
	\begin{definition}[Добавочное сопротивление.]
		Шунт к:
		\begin{itemize}
			\item Амперметру. $R = \frac{R_{A}}{n - 1}$.
			\item Вольтметру. $R = (n - 1) \cdot R_{V}$.
		\end{itemize}
	\end{definition}
	...
	\begin{enumerate}
		\item $\sum\limits_i I_i = 0$
		\item $\sum\limits_i \varepsilon_i = \sum\limits_i I_i R_i + \sum\limits_i I_i r_i$
	\end{enumerate}
	
	\subsection{Электрический ток в различных средах}
	
	\textbf{Вещества} делятся на:
	
	\begin{itemize}
		\item Проводники. Концентрация свободных электронов примерно $10^{28} \, \text{м}^{-3}$. Примеры: металлы, растворы и расплавы электролитов, плазма.
		
		\item Диэлектрики. Концентрация $< 10^{17} \, \text{м}^{-3}$. Примеры: янтарь, фарфор, резина, стекло, парафин, керосин, лаки, дистиллированная вода.
		
		\textbf{Пробивная напряженность}~--- напряженность, при которой диэлектрики начинают проводить ток. У воздуха: $3000 \dfrac{\text{кВ}}{\text{м}}$; фарфора: $8000-15000 \dfrac{\text{кВ}}{\text{м}}$.
		
		\item Полупроводники. Примеры: германий, селен, кремний.
	\end{itemize}
	
	\paragraph{Теория Друде} Уравнение движения свободного электрона в электрическом поле из классической теории электропроводности Друде, где связываются сила поля, дрейфовая скорость, время и длина свободного пробега электрона: $E \cdot \overline{e} = m \cdot a = \dfrac{m V_{max}}{\tau} = \dfrac{m V_{max} u}{\lambda}$, где $u = \sqrt{\dfrac{3KT}{m_e}} = \sqrt{\dfrac{3 \cdot 1.38 \cdot 10^{-23} \, \frac{\text{Дж}}{\text{К}} \cdot 300 \, \text{К}}{9.1 \cdot 10^{-31} \, \text{кг}}} \approx 1.168 \cdot 10^5 \, \dfrac{\text{м}}{\text{с}}$, $\tau = \dfrac{\lambda}{u}$.
	
	$E e = \dfrac{m u}{\lambda} \cdot \dfrac{2 I}{e n S} \Rightarrow j = E \cdot \dfrac{e^2 \lambda n}{2 m u} = E \sigma \Rightarrow I = \dfrac{\sigma S}{d} U \Rightarrow I = GU$.
	
	\begin{definition}[Проводимость, $G \, \text{См (симменсы)}$]
		$G = \dfrac{I}{U}$.
	\end{definition}
	
	\begin{definition}[Сверхпроводимость]
		Явление полной потери металлом электрического сопротивления при охлаждении до температуры, близкой к $0 \, \text{К}$.
	\end{definition}
	
	\subsection{Электрический ток в растворах и расплавах электролитов}
	
	\begin{definition}[Электролиты]
		Вещества, которые проводят ток из-за распада на ионы. Ионный тип проводимости.
	\end{definition}
	
	\begin{definition}[Электролитическая диссоциация]
		Явление процесса распада молекулы на ионы при растворении или плавлении.
	\end{definition}
	...
	\begin{definition}[Электро-химический эквивалент]
		$k = \dfrac{M}{N_a e z}$, $[k] = \dfrac{\text{кг}}{\text{Кл}}$.
	\end{definition}
	
	\begin{law}[Электролиза Фарадея]
		$m = k I \varDelta t$.
	\end{law}
	...
	\begin{definition}[Несамостоятельный электрический разряд]
		Явление протекания электрического тока через газ, наблюдаемое только при условиях кого-либо внешнего воздействия на газ.
	\end{definition}
	
	\begin{definition}[Плазма]
		Ионизирующий газ, в котором плотности положительных и отрицательных зарядов практически одинаковы.
	\end{definition}
	
	\subsubsection{Виды ионизации}
	
	\begin{itemize}
		\item Термическая.
		\item Фотоионизация.
		\item Ионизация электронным ударом.
	\end{itemize}
	
	\begin{definition}[Самостоятельный разряд]
		Явление протекающее в газе тока, не зависящего от внешних ионизаторов.
	\end{definition}
	
	\begin{definition}[Искровой разряд]
		Источник не может долго поддерживать самостоятельный заряд.
	\end{definition}
	
	\begin{definition}[Коронный разряд]
		При атмосферном давлении в сильно неоднородном электрическом поле.
	\end{definition}
	
	\begin{definition}[Тлеющий разряд]
		Достигается при пониженом давлении.
	\end{definition}
	
	\subsection{Электрический ток в вакууме}
	
	Пример: вакуумный диод.
	
	\subsection{Электрический ток в полупроводниках}
	
	\subsubsection{Примесная проводимость}
	
	\subsection{Магнетизм}
	
	\begin{definition}[Индукция магнитного поля]
		$B = \dfrac{F_{max}}{I l}$, $[B] =$ Тл.
	\end{definition}
	
	\begin{definition}[Сила Ампера]
		$F_A = B I l \sin \alpha$, $\alpha$~--- угол между полем и направлением тока в проводнике.
	\end{definition}
	
	\begin{definition}[Сила Лоренца]
		$F_L = B q V \sin \alpha$, $\alpha$~--- угол между полем и скоростью.
	\end{definition}
	
	\begin{law}[Био--Савара--Лапласа]
		$\varDelta B = \dfrac{\mu \mu_0 I \varDelta l \sin \alpha}{4 \pi r^2}$; $\mu_0 = 2 \pi \cdot 2 \cdot 10^{-7} = 1.26 \cdot 10^{-6} \dfrac{\text{Н}}{\text{А}^2}$.
	\end{law}
	
	\subsubsection{Поле внутри бесконечной катушки}

	Внутри бесконечной катушки поле однородное и равно $B = \dfrac{\mu \mu_0 I N}{l}$.
	
	\subsubsection{Электро--магнитная индукция}
	
	\begin{definition}[Поток вектора магнитной индукции]
		$\Phi = B S \cos (\widehat{\vec{B}, \vec{n}})$.
	\end{definition}
	
	\begin{law}[Фарадея]
		При изменении потока через замкнутый проводящий контур, в контуре возникает индукционный ток.
	\end{law}
	
	\begin{definition}[Вихревое электрическое поле]
		Такое поле, силовые линии которого замкнуты, а источник его возникновения~--- переменное магнитное поле. Это отличие от электростатического поля, силовые линии которого всегда начинаются на положительных зарядах и заканчиваются на отрицательных.
	\end{definition}
	
	\begin{definition}[ЭДС индукции, $\varepsilon_{ind}$]
		$\varepsilon_{ind} = -\Phi' (t)$.
	\end{definition}
	
	\paragraph{Самоиндукция}
	
	\begin{definition}[Индуктивность, $L \, \text{Гн}$ (Генри)]
		$L = \dfrac{\Phi}{I}$.
	\end{definition}
	...
	\subsubsection{Магнитные свойства вещества}
	
	\paragraph{Магнитная проницаемость}
	
	\begin{definition}[Магнитная проницаемость]
		$\mu = \dfrac{B}{B_0}$.
	\end{definition}
	
	\paragraph{Классификация веществ по магнитным свойствам}
	
	\begin{itemize}
		\item \textbf{Ферромагнетики.} $\mu >> 1$. Примеры: железо, кобальт, никель.
		\item \textbf{Парамагнетики.} $\mu > 1$. Примеры: соль железа, соль никеля.
		\item \textbf{Диамагнетики.} $\mu < 1$. Примеры: всякая органика, инертные газы.
	\end{itemize}
	
	\paragraph{Спин и домены}
	
	\begin{definition}[Спин]
		Собственный момент импульса элементарной частицы.
	\end{definition}
	
	\begin{definition}[Домены]
		Это намагниченные макроскопические участки с одинаковым направлением спинов.
	\end{definition}
	
	\paragraph{Намагниченность}
	
	\begin{definition}[Намагниченность]
		$J = (\mu - 1) B_0$.
	\end{definition}
    ...
    \begin{definition}[Гармонические колебания]
    	Колебания называются гармоническими, если возвращающая сила (а значит, и ускорение) пропорциональна смещению тела от положения равновесия и направлена против него.
    \end{definition}
	
	\subsection{Уравнение колебаний}
	
	\subsubsection{Пружинный маятник (горизонтальный)}
	
	Груз массой $m$ лежит на гладкой горизонтальной поверхности и прикреплен к пружине жесткости $k$; ось $Ox$ направлена вдоль движения, начало~--- в положении равновесия.
	
	\[ \vec{F}_{\text{упр}} + \vec{N} + m\vec{g} = m\vec{a} \]
	
	\[ Ox: \ F_{\text{упр }x} = ma_x \]
	
	\[-kx = ma_x\]
	
	\[ \omega_0^2 = \dfrac{k}{m} \text{~--- циклическая частота} \]
	
	\[ -\omega_0^2 x = a_x \]
	
	\subsubsection{Пружинный маятник (вертикальный)}
	
	Груз массой $m$ подвешен на пружине жесткости $k$. В положении равновесия пружина растянута на $|\varDelta l|$, при этом $F_{\text{упр}0} = k|\varDelta l|$, $\vec{mg} \downarrow$. Отклоним груз от положения равновесия на $\varDelta x$, тогда полное удлинение пружины равно $\varDelta x + |\varDelta l|$.
	
	\[ Ox: \ mg - k(\varDelta x + |\varDelta l|) = ma_x \]
	
	\[ \text{Так как } mg = k|\varDelta l| \text{, получаем } -kx = ma_x \text{, где } x = \varDelta x\]
	
	\begin{statement}
		Период колебаний пружинного маятника: $T = 2\pi \sqrt{\dfrac{m}{k}}$.
	\end{statement}
	
	\subsubsection{Математический маятник}
	
	Груз на нити длиной $l$ отклонен на угол $\alpha$ от вертикали ($\alpha$~--- мал), дуга $s = l\alpha \Rightarrow \alpha = \dfrac{s}{l}$. Проекция второго закона Ньютона на касательное направление:
	
	\[ -mg\sin\alpha = ma_{\tau} \]
	
	\[ \sin\alpha \approx \alpha \text{ (при малых углах)} \Rightarrow -g\alpha = a_{\tau} \]
	
	\[ -g\dfrac{s}{l} = a_{\tau} \]
	
	\[ \omega_0^2 = \dfrac{g}{l} \]
	
	\[ -\omega_0^2 s \approx a_{\tau} \]
	
	\begin{statement}
		Период колебаний математического маятника: $T = 2\pi \sqrt{\dfrac{l}{g}}$.
	\end{statement}
	
	\subsubsection{Обобщение}
	
	\begin{statement}
		Во всех трех случаях получено уравнение вида $a = -\omega_0^2 x$~--- это и есть уравнение гармонических колебаний, где $\omega_0$~--- циклическая (собственная) частота системы, $T = \dfrac{2\pi}{\omega_0}$.
	\end{statement}
	
    \subsubsection{Решение уравнения гармонических колебаний}
    
    \begin{statement}
    	Функция $x(t) = x_m \sin (\omega_0 t)$ является решением уравнения $x'' = -\omega_0^2 x$.
    \end{statement}
    \begin{proof}
    	$x = x_m \sin (\omega_0 t)$ \\
    	$x' = x_m \omega_0 \cos (\omega_0 t) =: V_x$ \\
    	$x'' = -x_m \omega_0^2 \sin (\omega_0 t) = -\omega_0^2 x$
    \end{proof}
    
    \begin{statement}[Общее решение]
    	Общим решением уравнения гармонических колебаний является
    	$$x = x_m \sin (\omega_0 t + \varphi_0),$$
    	где $x_m$~--- амплитуда, $\varphi_0$~--- начальная фаза. Проверка подстановкой полностью аналогична случаю $\varphi_0 = 0$ выше.
    \end{statement}
    
    \subsubsection{Нахождение амплитуды и начальной фазы по начальным условиям}
    
    \begin{problem}
    	Дано: $x_0$, $v_0$, $\omega_0$ ($v_{0x} > 0$). Найти: $x_m$, $\varphi_0$.
    \end{problem}
    
    \begin{solution}
    	Подставим $t = 0$ в $x(t)$ и $x'(t)$:
    	$$x_0 = x_m \sin (\varphi_0), \qquad v_0 = x_m \omega_0 \cos (\varphi_0).$$
    	Возведем оба равенства в квадрат и сложим:
    	$$x_0^2 + \frac{v_0^2}{\omega_0^2} = x_m^2(\sin^2(\varphi_0) + \cos^2(\varphi_0)) = x_m^2 \; \Rightarrow \; x_m = \sqrt{x_0^2 + \frac{v_0^2}{\omega_0^2}}.$$
    	Разделим первое равенство на второе:
    	$$\frac{x_0 \omega_0}{v_0} = \tg (\varphi_0) \; \Rightarrow \; \varphi_0 = \arctg \left( \frac{x_0 \omega_0}{v_0} \right).$$
    \end{solution}
\end{document}
