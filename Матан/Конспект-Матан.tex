...
\begin{document}
	...
	\begin{enumerate}
		...
		Пусть $f$ и $g$ интегрируемы на $[a, b]$, тогда $\forall \alpha, \beta$ $\alpha f + \beta g$ тоже интегрируема на $[a, b]$ и $\dint\limits_a^b (\alpha f + \beta g) dx = \underbrace{\alpha \dint\limits_a^bf dx}_A + \underbrace{\beta \dint\limits_a^b g dx}_B$.
		
		\begin{proof}
			fix $\varepsilon$.
			
			$\left| \sum (\alpha f(\xi_k) + \beta g(\xi_k)) \varDelta x - \alpha A - \beta B \right|$
		\end{proof}
	\end{enumerate}
