...
\begin{document}
	...
	\begin{definition}[Интеграл функции]
		Пусть $\exists I \in \mathbb{R}$, такое что $\forall \varepsilon > 0 \, \exists \delta > 0$: $\forall$ разбиения $d < \delta$, $\forall$ выбора $\{ \xi_k \}$ верно $\sum\limits_{k = 1}^n f(\xi_k)(x_k - x_{k - 1}) - I < \varepsilon$, тогда $I := \int\limits_a^b f(x) dx$.
	\end{definition}
	...
	\begin{lemma}
		$S^* = S_* \Leftrightarrow \exists \int\limits_a^b f(x) dx$.
	\end{lemma}
	...
	\begin{theorem}[Вейерштрасса]
		Пусть $f \in C([a; b])$. Тогда $\exists \, \int\limits_a^b f(x) dx$.
	\end{theorem}
	...
	\begin{proof}
		...
		$0 \leqslant \overline{S}_{\{ x_k\}} - \underline{S}_{\{ x_k \}} < \varepsilon (b - a) \Rightarrow S^* = S_* \Rightarrow \exists \int\limits_a^b f(x) dx$.
	\end{proof}
\end{document}
