...
\begin{document}
	...
	Рассмотрим ограниченную функцию $f: [a, b] \rightarrow \mathbb{R}, \, m \leqslant f(x) \leqslant M$. Возьмем точки $\{ x_0 = a < x_1 < x_2 < \dots < x_n = b \}$. $\xi_k \in [x_{k - 1}, x_k], \, \forall k = 1 \dots n$.
	
	\begin{definition}[Разбиение отрезка с выбранными точками]
		Набор $x_k$ с набором $\xi_k$ ($\forall k = 1 \dots n$) называется разбиеним отрезка с выбранными точками.
	\end{definition}
	
	\begin{definition}[Интегральная сумма функции $f$ для разбиения отрезка с выбранными точками]
		$$ m (b - a) \leqslant \sum\limits_{k = 1}^n f(\xi_k)(x_k - x_{k - 1}) \leqslant M (b - a) $$
	\end{definition}
	
	\begin{definition}[Мелкость (ранг) разбиения]
		$$ d = \max\limits_{k = 1}^n \{ x_k - x_{k - 1} \} $$
	\end{definition}
	
	\begin{definition}[Интеграл функции]
		Пусть $\exists I \in \mathbb{R}$, такое что $\forall \varepsilon > 0 \, \exists \delta > 0$: $\forall$ разбиения $d < \delta$, $\forall$ выбора $\{ \xi_k \}$ верно $\sum\limits_{k = 1}^n f(\xi_k)(x_k - x_{k - 1}) - I < \varepsilon$, тогда $I := \dint\limits_a^b f(x) dx$.
	\end{definition}
	
	\begin{definition}[Верхняя и нижняя сумма Дарбу]
		$f$ ограничена на $[a; b] \, \{ x_k \}_{k = 0}^n$~--- разбиение.
		
		$\sum\limits_{k = 1}^n \sup f_{[x_{k - 1}, x_k]} \varDelta x_k = \overline{S}_{\{ x_k \}}$~--- верхняя сумма Дарбу.
		
		$\sum\limits_{k = 1}^n \inf f_{[x_{k - 1}, x_k]} \varDelta x_k = \underline{S}_{\{ x_k \}}$~--- нижняя сумма Дарбу.
	\end{definition}
	
	\begin{statement}
		$\forall$ выбора $\{ \xi_k \}$ (для fix $\{ x_k \}$): $$ \underline{S}_{\{ x_k \}} \leqslant \sum\limits_{k = 1}^n f(\xi_k) \varDelta x_k \leqslant \overline{S}_{\{ x_k \}} $$
	\end{statement}
	
	\begin{lemma}
		Если добавить в разбиение еще одну точку, то верхняя сумма Дарбу не увеличится, а нижняя~--- не уменьшится.
	\end{lemma}
	
	\begin{proof}
		Возьмем точку $c$.
		
		$\sup f_{[x_k; x_{k + 1}]} \varDelta x_k \rightarrow \sup f_{[x_k, c]} (c - x_k) + \sup f_{[c; x_{k + 1}]} (x_{k + 1} - c) \leqslant \sup f_{[x_k, x_{k + 1}]} (c - x_k) + \sup f_{[x_k, x_{k + 1}]} (x_{k + 1} - c) = \sup f_{[x_k, x_{k + 1}]} (x_{k + 1} - x_k)$.
		
		Для минимума аналогично.
	\end{proof}
	
	\begin{lemma}
		$\{ x_k \}, \, \{ y_m \}$~--- два разбиения $[a; b]$. Тогда $\underline{S}_{\{ x_k \}} \leqslant \overline{S}_{\{ y_m \}}$.
	\end{lemma}
	
	\begin{statement}
		$\{ \overline{S}_{\{ x_k \}} \}_{\forall \text{разбиения}}$ ограничено снизу; $\{ \underline{S}_{\{ x_k \}} \}_{\forall \text{разбиения}}$ ограничено сверху.
	\end{statement}
	
	\begin{definition}
		$S^* = \inf \overline{S}_{\{ x_k \}}$; $S_* = \sup \underline{S}_{\{ x_k \}}$. $\inf$ и $\sup$ берутся по множеству всех разбиений $[a; b]$.
	\end{definition}
	
%	\begin{lemma}
%		$S^* = S_* \Leftrightarrow \exists \dint\limits_a^b f(x) dx$.
%	\end{lemma}
%	
%	\begin{proof}
%		\begin{itemize}
%			\item[$\Rightarrow$]
%			
%			Пусть $S^* = S_* = S$. fix $\varepsilon > 0$.
%			
%			$\exists \{ z_l \}$: $\overline{S}_{\{ z_l \}} < S + \varepsilon$.
%			
%			$\exists \{ y_m \}$: $\underline{S}_{\{ y_m \}} > S - \varepsilon$.
%			
%			$\rightarrow \{ x_k \} := \{ z_l \} \cup \{ y_m \}$.
%			
%			$S - \varepsilon < \underline{S}_{\{ x_k \}} \leqslant \overline{S}_{\{ x_k \}} < S + \varepsilon$; $\delta := \max \{ \varDelta x_k \}$~--- искомая $\delta$.
%			
%			$\forall$ разбиения $\{ w_m \}$, мелкость $d < \delta$; $\{ \tilde{w}_m \} = \{ w_m \} \cup \{ x_k \}$.
%			
%			$\underline{S}_{\{ w_m \}} \leqslant \underline{S}_{\{ \tilde{w}_m \}} \leqslant \overline{S}_{\{ \tilde{w}_m \}} \leqslant \overline{S}_{\{ w_m \}}$ и $\overline{S}_{\{ \tilde{w}_m \}} - \underline{S}_{\{ \tilde{w}_m \}} \leqslant \overline{S}_{x_k} - \underline{S}_{x_k} < 2 \varepsilon$
%			
%			$\left| \sum\limits_{m = 1}^m f(\xi_m) \varDelta w_m - S \right| \leqslant \max \{ \left| \sum \dots - \underline{S}_{\{ w_m \}} \right|; \left| \sum \dots - \overline{S}_{\{ w_m \}} \right| \} \overset{?}{\leqslant} \overline{S}_{\{ w_m \}} - \underline{S}_{\{ w_m \}} < 2 \varepsilon$
%			
%			\item[$\Leftarrow$]
%			
%			fix $\varepsilon > 0$. $\exists \{ x_k \}$ $\forall$ выбора точек $\xi_k$ $\sum f(\xi_k) \varDelta x_k < S + \varepsilon$.
%			
%			Пусть $\xi_k: f(\xi_k) > \sup f_{[x_{k - 1}, x_k]} - \dfrac{\varepsilon}{2 (b - a)}$.
%			
%			$\sum f(\xi_k) \varDelta x_k \geqslant \sum \left( \sup f_{[x_{k - 1}, x_k]} - \dfrac{\varepsilon}{2 (b - a)} \right) \varDelta x_k$
%		\end{itemize}
%	\end{proof}
    ...
    \begin{proof}
        \begin{itemize}
            ...
            Пусть $S^* = S_* = S$. Докажем по критерию Римана, что
            \[
                \exists \int\limits_a^b f(x)\,dx = S.
            \]
            
            Зафиксируем $\varepsilon > 0$.
            
            По определению точной верхней грани существует разбиение $\{z_l\}$ такое, что
            \[
                \overline S_{\{z_l\}} < S + \varepsilon.
            \]
            По определению точной нижней грани существует разбиение $\{y_m\}$ такое, что
            \[
                \underline S_{\{y_m\}} > S - \varepsilon.
            \]
            
            Рассмотрим их общее измельчение
            \[
                \{x_k\} := \{z_l\} \cup \{y_m\}.
            \]
            Тогда, поскольку при измельчении нижняя сумма не убывает, а верхняя не возрастает,
            \[
                S - \varepsilon
                < \underline S_{\{y_m\}}
                \le \underline S_{\{x_k\}}
            ...
            Следовательно,
            \[
                \overline S_{\{x_k\}} - \underline S_{\{x_k\}} < 2\varepsilon.
            \]
            
            Обозначим
            \[
                \delta := \min_{k} (x_k - x_{k-1}) > 0.
            \]
            Покажем, что эта $\delta$ подходит в критерии интегрируемости.
            
            Пусть $\{w_j\}$ --- любое разбиение отрезка $[a,b]$ мелкости
            \[
                d(\{w_j\}) := \max_j (w_j - w_{j-1}) < \delta.
            \]
            Тогда каждое звено разбиения $\{w_j\}$ не может содержать более одной внутренней точки разбиения $\{x_k\}$; поэтому разбиение
            \[
                \{\widetilde w_j\} := \{w_j\} \cup \{x_k\}
            \]
            получается из $\{w_j\}$ добавлением точек, и на каждом промежутке разбиения $\{w_j\}$ точки разбиения $\{x_k\}$ лишь уточняют его.
            
            Для любого выбора отмеченных точек $\xi_j \in [w_{j-1},w_j]$ имеем
            \[
                \underline S_{\{w_j\}}
                \le \sum_j f(\xi_j)\,\Delta w_j
                \le \overline S_{\{w_j\}}.
            \]
            Кроме того,
            \[
                \underline S_{\{w_j\}}
                \le \underline S_{\{\widetilde w_j\}}
                \le \overline S_{\{\widetilde w_j\}}
                \le \overline S_{\{w_j\}}.
            \]
            Так как $\{\widetilde w_j\}$ является измельчением разбиения $\{x_k\}$, то
            \[
                \overline S_{\{\widetilde w_j\}} - \underline S_{\{\widetilde w_j\}}
                \le
                \overline S_{\{x_k\}} - \underline S_{\{x_k\}}
                < 2\varepsilon.
            \]
            Отсюда
            \[
                S-\varepsilon < \underline S_{\{x_k\}}
                \le \underline S_{\{\widetilde w_j\}}
                \le \sum_j f(\xi_j)\,\Delta w_j
                \le \overline S_{\{\widetilde w_j\}}
                \le \overline S_{\{x_k\}} < S+\varepsilon,
            \]
            то есть
            \[
                \left|\sum_j f(\xi_j)\,\Delta w_j - S\right| < \varepsilon.
            \]
            
            Значит, по критерию Римана функция $f$ интегрируема на $[a,b]$, и
            \[
                \int_a^b f(x)\,dx = S.
            \]
            ...
            Зафиксируем $\varepsilon > 0$. По критерию Римана существует $\delta > 0$ такое, что для всякого разбиения
            \[
                \{x_k\}, \qquad d(\{x_k\}) < \delta,
            \]
            и для любого выбора точек $\xi_k \in [x_{k-1},x_k]$ выполнено
            \[
                \left|\sum_k f(\xi_k)\,\Delta x_k - S\right| < \frac{\varepsilon}{2}.
            \]
            
            Возьмём какое-нибудь разбиение $\{x_k\}$ мелкости $<\delta$.
            
            Для каждого $k$ выберем точку $\xi_k$ так, чтобы
            \[
                f(\xi_k) > \sup_{[x_{k-1},x_k]} f - \frac{\varepsilon}{2(b-a)}.
            \]
            Тогда
            \[
                \sum_k f(\xi_k)\,\Delta x_k
                \ge
                \sum_k \left(\sup_{[x_{k-1},x_k]} f - \frac{\varepsilon}{2(b-a)}\right)\Delta x_k
                =
                \overline S_{\{x_k\}} - \frac{\varepsilon}{2(b-a)}\sum_k \Delta x_k
                =
                \overline S_{\{x_k\}} - \frac{\varepsilon}{2}.
            \]
            С другой стороны,
            \[
                \sum_k f(\xi_k)\,\Delta x_k < S + \frac{\varepsilon}{2}.
            \]
            Поэтому
            \[
                \overline S_{\{x_k\}} < S + \varepsilon.
            \]
            Значит,
            \[
                S^* \le \overline S_{\{x_k\}} < S + \varepsilon.
            \]
            
            Аналогично, выбирая точки $\eta_k$ так, что
            \[
                f(\eta_k) < \inf_{[x_{k-1},x_k]} f + \frac{\varepsilon}{2(b-a)},
            \]
            получаем
            \[
                \sum_k f(\eta_k)\,\Delta x_k
                \le
                \underline S_{\{x_k\}} + \frac{\varepsilon}{2}.
            \]
            Но одновременно
            \[
                \sum_k f(\eta_k)\,\Delta x_k > S - \frac{\varepsilon}{2},
            \]
            следовательно,
            \[
                \underline S_{\{x_k\}} > S - \varepsilon.
            \]
            Значит,
            \[
                S_* \ge \underline S_{\{x_k\}} > S - \varepsilon.
            \]
            
            Так как для любого разбиения всегда
            \[
                \underline S_{\{x_k\}} \le S_* \le S^* \le \overline S_{\{x_k\}},
            \]
            то из полученных оценок следует
            \[
                S - \varepsilon < S_* \le S^* < S + \varepsilon.
            \]
            Поскольку $\varepsilon > 0$ произвольно, заключаем:
            \[
                S_* = S^* = S.
            \]
        \end{itemize}
	...
	\begin{theorem}[Вейерштрасса]
		Пусть $f \in C([a; b])$. Тогда $\exists \, \dint\limits_a^b f(x) dx$.
	\end{theorem}
	
	\begin{proof}
		fix $\varepsilon > 0$. $f$ равномерно непрерывна на $[a; b] \Rightarrow \exists \delta = \delta(\varepsilon): \, \forall x, y \in [a; b]: \, |x - y| < \delta \Rightarrow |f(x) - f(y)| < \varepsilon$.
		
		Возьмем произвольное разбиение $\{ x_k \}$, $d < \delta$.
		
		Пусть $\xi_k$~--- точки максимума, $\eta_k$~--- точки минимум а на $[x_{k - 1}, x_k]$.
		
		$\overline{S}_{\{ x_k \}} - \underline{S}_{\{ x_k \}} = \sum\limits_{k = 1}^k (f(\xi_k) - f(\eta_k)) \varDelta x_k < \varepsilon \sum\limits_{k = 1}^k \varDelta x_k = \varepsilon (b - a)$.
		
		$0 \leqslant \overline{S}_{\{ x_k\}} - \underline{S}_{\{ x_k \}} < \varepsilon (b - a) \Rightarrow S^* = S_* \Rightarrow \exists \dint\limits_a^b f(x) dx$.
	\end{proof}
	...
	\begin{enumerate}
		...
		Пусть $f$ и $g$ интегрируемы на $[a, b]$, тогда $\forall \alpha, \beta \in \mathbb{R}$ функция $\alpha f + \beta g$ тоже интегрируема на $[a, b]$ и 
		$$ \dint\limits_a^b (\alpha f(x) + \beta g(x)) dx = \alpha \dint\limits_a^b f(x) dx + \beta \dint\limits_a^b g(x) dx $$
		
		\begin{proof}
			Пусть $A = \dint\limits_a^b f(x) dx$ и $B = \dint\limits_a^b g(x) dx$. Рассмотрим модуль разности интегральной суммы и предполагаемого значения интеграла:
			\begin{align*}
				&\left| \sum\limits_{k = 1}^n (\alpha f(\xi_k) + \beta g(\xi_k))\varDelta x_k - (\alpha A + \beta B) \right| \\
				={}& \left| \alpha \left(\sum\limits_{k = 1}^n f(\xi_k)\varDelta x_k - A\right) + \beta \left(\sum\limits_{k = 1}^n g(\xi_k)\varDelta x_k - B\right) \right| \\
				\leqslant{}& |\alpha| \cdot \left| \sum\limits_{k = 1}^n f(\xi_k)\varDelta x_k - A \right| + |\beta| \cdot \left| \sum\limits_{k = 1}^n g(\xi_k)\varDelta x_k - B \right|
			\end{align*}
			Fix $\varepsilon > 0$. Так как $f$ и $g$ интегрируемы, то для $\varepsilon' = \dfrac{\varepsilon}{|\alpha| + |\beta| + 1} > 0$ существуют $\delta_1 > 0$ и $\delta_2 > 0$ такие, что для любого разбиения с мелкостью $d < \delta_1$ (для $f$) и $d < \delta_2$ (для $g$) выполняется:
			$$ \left| \sum f(\xi_k)\varDelta x_k - A \right| < \varepsilon' \quad \text{и} \quad \left| \sum g(\xi_k)\varDelta x_k - B \right| < \varepsilon' $$
			Выберем $\delta = \min(\delta_1, \delta_2)$. Тогда для любого разбиения с мелкостью $d < \delta$ имеем:
			$$ |\alpha| \cdot \left| \dots \right| + |\beta| \cdot \left| \dots \right| < |\alpha|\varepsilon' + |\beta|\varepsilon' = (|\alpha| + |\beta|)\frac{\varepsilon}{|\alpha| + |\beta| + 1} < \varepsilon $$
			Что и требовалось доказать.
		\end{proof}
		
		\item \textbf{Аддитивность.}
		
		Пусть $f$ интегрируема на $[a, b]$. Тогда $\forall c \in (a, b)$ функция $f$ интегрируема на отрезках $[a, c]$ и $[c, b]$, и справедливо равенство:
		$$ \dint\limits_a^b f(x) dx = \dint\limits_a^c f(x) dx + \dint\limits_c^b f(x) dx $$
		
		\item \textbf{Монотонность.}
		
		Пусть функции $f(x)$ и $g(x)$ интегрируемы на $[a, b]$ и $\forall x \in [a, b]$ выполняется $f(x) \leqslant g(x)$. Тогда:
		$$ \dint\limits_a^b f(x) dx \leqslant \dint\limits_a^b g(x) dx $$
		Если, кроме того, $f$ и $g$ непрерывны и $\exists x_0 \in [a, b]: f(x_0) < g(x_0)$, то неравенство будет строгим.
		
		\begin{proof}[Идея]
			Рассмотрим функцию $h(x) = g(x) - f(x) \geqslant 0$. Интегральная сумма для $h(x)$ будет неотрицательна, следовательно, и ее предел (интеграл) также будет неотрицателен: $\dint\limits_a^b (g(x) - f(x)) dx \geqslant 0$.
		\end{proof}
		
	\end{enumerate}
	
	\begin{theorem}[О среднем значении в интегральной форме]
		Пусть $f(x)$ непрерывна на отрезке $[a, b]$. Тогда $\exists c \in [a, b]$ такая, что
		$$ \dint\limits_a^b f(x) dx = f(c) \cdot (b - a) $$
	\end{theorem}
	\begin{proof}
		Т.к. $f \in C([a, b])$, по теореме Вейерштрасса она достигает на нем своих наименьшего $m$ и наибольшего $M$ значений.
		$$ m \leqslant f(x) \leqslant M, \quad \forall x \in [a, b] $$
		Интегрируя это неравенство от $a$ до $b$:
		$$ \dint\limits_a^b m \,dx \leqslant \dint\limits_a^b f(x) \,dx \leqslant \dint\limits_a^b M \,dx $$
		$$ m(b - a) \leqslant \dint\limits_a^b f(x) \,dx \leqslant M(b - a) $$
		Разделим на $(b - a)$:
		$$ m \leqslant \frac{1}{b - a}\dint\limits_a^b f(x) \,dx \leqslant M $$
		По теореме о промежуточном значении для непрерывной функции, существует точка $c \in [a, b]$, в которой функция принимает это среднее значение.
	\end{proof}
	
	\begin{definition}[Кусочно-непрерывная функция]
		Функция называется \textbf{кусочно-непрерывной} на отрезке $[a, b]$, если она имеет конечное число точек разрыва, и все они являются разрывами первого рода.
	\end{definition}
	...
	Пусть $f$ интегрируема на $[\alpha, \beta]$. Зафиксируем $a \in [\alpha, \beta]$ и рассмотрим функцию:
	$$ F(x) = \dint\limits_a^x f(t) dt, \quad x \in [\alpha, \beta] $$
	
	\begin{theorem}[Основная теорема анализа (Теорема Барроу)]
		Пусть $f(t)$ непрерывна на отрезке $[\alpha, \beta]$. Тогда функция $F(x) = \dint\limits_a^x f(t) dt$:
		\begin{enumerate}
			\item Непрерывна на $[\alpha, \beta]$.
			\item Дифференцируема в любой точке $x_0 \in (\alpha, \beta)$, и ее производная равна $F'(x_0) = f(x_0)$.
		\end{enumerate}
	\end{theorem}
	\begin{proof}
		1) Докажем непрерывность. Рассмотрим $\lim_{\Delta x \to 0} (F(x_0 + \Delta x) - F(x_0))$.
		$$ F(x_0 + \Delta x) - F(x_0) = \dint\limits_a^{x_0 + \Delta x} f(t) dt - \dint\limits_a^{x_0} f(t) dt = \dint\limits_{x_0}^{x_0 + \Delta x} f(t) dt $$
		Так как $f$ непрерывна на отрезке, она на нем ограничена. $|\dint\limits_{x_0}^{x_0 + \Delta x} f(t) dt| \leqslant \sup |f| \cdot |\Delta x| \to 0$ при $\Delta x \to 0$.
		
		2) Найдем производную.
		$$ F'(x_0) = \lim_{\Delta x \to 0} \frac{F(x_0 + \Delta x) - F(x_0)}{\Delta x} = \lim_{\Delta x \to 0} \frac{1}{\Delta x} \dint\limits_{x_0}^{x_0 + \Delta x} f(t) dt $$
		По теореме о среднем, $\exists c \in [x_0, x_0 + \Delta x]$ такое, что $\dint\limits_{x_0}^{x_0 + \Delta x} f(t) dt = f(c) \cdot \Delta x$.
		$$ F'(x_0) = \lim_{\Delta x \to 0} \frac{f(c) \cdot \Delta x}{\Delta x} = \lim_{\Delta x \to 0} f(c) = f(x_0) $$
		Последний переход верен, так как при $\Delta x \to 0$ имеем $c \to x_0$, а функция $f$ непрерывна.
	\end{proof}
	
	\begin{definition}[Первообразная]
		Пусть $f$ определена на $[\alpha, \beta]$. Функция $F(x)$ называется \textbf{первообразной} для $f(x)$, если $\forall x \in [\alpha, \beta]$ выполняется $F'(x) = f(x)$.
	\end{definition}
	
	\begin{statement}
		Из теоремы Барроу следует, что если функция $f$ непрерывна на отрезке, то у нее существует хотя бы одна первообразная (а именно, $F(x) = \dint_a^x f(t)dt$).
	\end{statement}
	
	\begin{lemma}
		Пусть $f$ непрерывна на $[\alpha, \beta]$; $F_1(x)$ и $F_2(x)$~--- две первообразные к $f(x)$ на $[\alpha, \beta]$. Тогда $F_1(x) - F_2(x) = const$.
	\end{lemma}
	
	\begin{proof}
		$G(x) = F_1(x) - F_2(x)$; $\forall x_1, x_2 \in [\alpha, \beta]$.
		
		По теореме Лагранжа о средних в дифференциальной форме:
		
		$\exists c \in (x_1, x_2)$: $G(x_1) - G(x_2) = G'(c)(x_1 - x_2) = (F_1'(c) - F_2'(c))(x_1 - x_2) = 0 \, \forall x_1, x_2 \Rightarrow G(x) = const$.
	\end{proof}
	
	\begin{theorem}[Формула Ньютона-Лейбница]
		$f$ непрерывна на $[a, b]$; $F(x)$~--- любая первообразная $f(x)$. Тогда $\dint\limits_a^b f(x) dx = F(b) - F(a) =: F(x) \Bigr|_a^b$.
	\end{theorem}
	
	\begin{proof}
		$\Phi (x) = \dint\limits_a^b f(x) dx = F(x) + C$.
		
		$t = b$: $\Phi (b) = F(b) + C$.
		
		$t = a$: $\Phi (a) = F(a) + C \Rightarrow C = -F(a)$.
	\end{proof}
	
	\begin{theorem}[Формула замены переменной]
		Пусть $f$ непрерывна на $[\alpha, \beta]$; $g$ непрерывно дифференцируема на $[a, b]$ и $g(a) = \alpha$, $g(b) = \beta$. Тогда $\dint\limits_a^b f(g(x)) g'(x) dx = \dint\limits_{g(a)}^{g(b)} f(t) dt$.
	\end{theorem}
	
	\begin{proof}
		$\dint\limits_a^b f(t) dt = F(\beta) - F(\alpha) = F(g(b)) - F(g(a)) = F(g(x)) \Bigr|_a^b = \dint\limits_a^b (F(g(x)))' dx\\
		= \dint\limits_a^b f(g(x)) g'(x) dx$.
	\end{proof}
	
	\begin{theorem}[Формула интегрирования по частям]
		Пусть $f$ и $g$ непрерывно дифференцируемы на $[a, b]$. Тогда $\dint\limits_a^b f(x) g'(x) dx = f(x) g(x) \Bigr|_a^b - \dint\limits_a^b f'(x) g(x) dx$, что эквивалентно $\dint\limits_a^b f(x) d g(x) = f(x) g(x) \Bigr|_a^b - \dint\limits_a^b g(x) d f(x)$.
	\end{theorem}
	
	\begin{proof}
		$\dint\limits_a^b f g' dx + \dint\limits_a^b f' g dx = \dint\limits_a^b (f g' + f' g) dx = \dint\limits_a^b (fg)' dx = fg \Bigr|_a^b$.
	\end{proof}
	
	\subsection{Правила интегрирования.}
	
    \begin{enumerate}
        \item $\dint x^s\,dx = \dfrac{x^{s+1}}{s+1} + C$; $s \ne -1$
        
        \textbf{Вывод:} $\left(\dfrac{x^{s+1}}{s+1}\right)' = \dfrac{(s+1)x^s}{s+1} = x^s$. \checkmark
        
        \item $\dint \dfrac{dx}{x} = \ln|x| + C$
        
        \textbf{Вывод:} При $x > 0$: $(\ln x)' = \dfrac{1}{x}$. При $x < 0$: $(\ln(-x))' = \dfrac{-1}{-x} = \dfrac{1}{x}$. Итого $(\ln|x|)' = \dfrac{1}{x}$. \checkmark
        
        \item $\dint a^x\,dx = \dfrac{a^x}{\ln a} + C$; $\dint e^x\,dx = e^x + C$
        
        \textbf{Вывод:} $\left(\dfrac{a^x}{\ln a}\right)' = \dfrac{a^x \ln a}{\ln a} = a^x$. При $a = e$: $\ln e = 1$, поэтому $(e^x)' = e^x$. \checkmark
        
        \item $\dint \sin x\,dx = -\cos x + C$
        
        \textbf{Вывод:} $(-\cos x)' = \sin x$. \checkmark
        
        \item $\dint \cos x\,dx = \sin x + C$
        
        \textbf{Вывод:} $(\sin x)' = \cos x$. \checkmark
        
        \item $\dint \dfrac{dx}{\sin^2 x} = -\ctg x + C$
        
        \textbf{Вывод:} $(-\ctg x)' = -\left(-\dfrac{1}{\sin^2 x}\right) = \dfrac{1}{\sin^2 x}$. \checkmark
        
        \item $\dint \dfrac{dx}{\cos^2 x} = \tg x + C$
        
        \textbf{Вывод:} $(\tg x)' = \left(\dfrac{\sin x}{\cos x}\right)' = \dfrac{\cos^2 x + \sin^2 x}{\cos^2 x} = \dfrac{1}{\cos^2 x}$. \checkmark
        
        \item $\dint \tg x\,dx = -\ln|\cos x| + C$
        
        \textbf{Вывод:}
        \[
            \dint \tg x\,dx
            = \dint \frac{\sin x}{\cos x}\,dx
            = -\dint \frac{d(\cos x)}{\cos x}
            = -\ln|\cos x| + C.
        \]
        
        \item $\dint \ctg x\,dx = \ln|\sin x| + C$
        
        \textbf{Вывод:}
        \[
            \dint \ctg x\,dx
            = \dint \frac{\cos x}{\sin x}\,dx
            = \dint \frac{d(\sin x)}{\sin x}
            = \ln|\sin x| + C.
        \]
        
        \item $\dint \dfrac{dx}{x^2 + 1} = \arctg x + C$
        
        \textbf{Вывод:} $(\arctg x)' = \dfrac{1}{x^2 + 1}$. \checkmark
        
        Частный случай:
        \[
            \dint \frac{dx}{x^2 + a^2}
            = \frac{1}{a^2}\dint \frac{dx}{\left(\tfrac{x}{a}\right)^2 + 1}
            = \frac{1}{a^2}\dint \frac{a\,d\!\left(\tfrac{x}{a}\right)}{\left(\tfrac{x}{a}\right)^2 + 1}
            = \frac{1}{a}\arctg\frac{x}{a} + C.
        \]
        
        \item $\dint \dfrac{dx}{x^2 - a^2} = \dfrac{1}{2a}\ln\left|\dfrac{x-a}{x+a}\right| + C$
        
        \textbf{Вывод:} Разложим дробь на простейшие:
        \[
            \frac{1}{x^2 - a^2}
            = \frac{1}{(x-a)(x+a)}
        ...
        \[
            \dint \frac{dx}{x^2 - a^2}
            = \frac{1}{2a}\left(\dint\frac{d(x-a)}{x-a} - \dint\frac{d(x+a)}{x+a}\right)
            = \frac{1}{2a}\ln\left|\frac{x-a}{x+a}\right| + C.
        \]
        
        \item $\dint \dfrac{dx}{\sqrt{1 - x^2}} = \arcsin x + C$
        
        \textbf{Вывод:} $(\arcsin x)' = \dfrac{1}{\sqrt{1-x^2}}$. \checkmark
        
        \item $\dint \dfrac{dx}{\sqrt{x^2 - 1}} = \operatorname{arcsinh} x + C$
        
        \textbf{Вывод:} Сделаем замену $x = \cosh t$, $dx = \sinh t\,dt$, $\sqrt{x^2-1} = \sinh t$:
        \[
            \dint\frac{dx}{\sqrt{x^2-1}}
            = \dint\frac{\sinh t\,dt}{\sinh t}
            = \dint dt
            = t + C
            = \operatorname{arccosh} x + C
            = \ln\!\left(x + \sqrt{x^2 - 1}\right) + C.
        \]
        
        \item $\dint \dfrac{dx}{\sqrt{x^2+1}} = \ln\!\left(x + \sqrt{x^2+1}\right) + C$
        
        \textbf{Вывод:} Сделаем замену $x = \sinh t$, $dx = \cosh t\,dt$, $\sqrt{x^2+1} = \cosh t$:
        \[
            \dint\frac{dx}{\sqrt{x^2+1}}
            = \dint\frac{\cosh t\,dt}{\cosh t}
            = \dint dt
            = t + C
        ...
        \item $\dint \arctg x\,dx = x\arctg x - \dfrac{1}{2}\ln(x^2+1) + C$
        
        \textbf{Вывод:} Интегрируем по частям ($u = \arctg x$, $dv = dx$):
        \[
            \dint \arctg x\,dx
            = x\arctg x - \dint x\,d(\arctg x)
            = x\arctg x - \dint \frac{x\,dx}{x^2+1}
            = x\arctg x - \frac{1}{2}\dint\frac{d(x^2+1)}{x^2+1}
	...
	\subsubsection{Объемы тел вращения}
	\begin{statement}
		Объем тела вращения, образованного вращением вокруг оси $Ox$ криволинейной трапеции, ограниченной графиком непрерывной функции $y = f(x)$, равен:
		$$ V_{вр} = \pi \dint_a^b f^2(x) dx $$
	\end{statement}
	\noindent
	Пример: $y = \sin x$ на $[0, \pi]$. Тогда $V = \pi \dint_0^\pi \sin^2 x dx = \frac{\pi^2}{2}$.

	\subsubsection{Длина дуги кривой}
	\begin{statement}
		Пусть функция $f(x)$ непрерывно дифференцируема на $[a, b]$. Тогда длина $L$ дуги кривой $y = f(x)$ от $a$ до $b$ вычисляется по формуле:
		$$ L = \dint_a^b \sqrt{1 + (f'(x))^2} dx $$
	\end{statement}
	\begin{proof}[Идея вывода]
		Разобьем отрезок на части. Длина малого участка $\Delta L_k \approx \sqrt{\Delta x_k^2 + \Delta y_k^2} = \sqrt{1 + (\frac{\Delta y_k}{\Delta x_k})^2} \Delta x_k$. По теореме Лагранжа $\frac{\Delta y_k}{\Delta x_k} = f'(\xi_k)$. Переходя к пределу при $\max \Delta x_k \rightarrow 0$, получаем интеграл.
	\end{proof}

	\noindent
	Если кривая в $\mathbb{R}^3$ задана параметрически $\begin{cases} x = x(t) \\ y = y(t) \\ z = z(t) \end{cases}, t \in [t_1, t_2]$, то:
	$$ L = \dint_{t_1}^{t_2} \sqrt{(x'(t))^2 + (y'(t))^2 + (z'(t))^2} dt $$

	\subsubsection{Площадь в параметрическом виде}
	\begin{statement}
		Пусть кривая задана параметрически $x = x(t), y = y(t), t \in [t_1, t_2]$. Тогда площадь фигуры, ограниченной этой кривой, вычисляется через криволинейный интеграл:
		$$ S = \dint_{t_1}^{t_2} y(t) x'(t) dt \quad \text{или} \quad S = \frac{1}{2} \dint_{t_1}^{t_2} (x y' - y x') dt $$
	\end{statement}
	\noindent
	Пример: Площадь эллипса $\frac{x^2}{a^2} + \frac{y^2}{b^2} = 1$. Параметризация: $x = a \cos t, y = b \sin t, t \in [0, 2\pi]$.
	$$ S = \frac{1}{2} \dint_0^{2\pi} (ab \cos^2 t + ab \sin^2 t) dt = \frac{1}{2} ab \dint_0^{2\pi} 1 dt = \pi ab $$

    \subsection{Полярные координаты}

    \begin{statement}
        Пусть у нас есть график в полярных координатах $r = r(\varphi)$. Тогда его площадь равна $$S = \dfrac{1}{2} \dint_{\varphi_1}^{\varphi_2} r^2(\varphi) d \varphi$$
    \end{statement}

    \begin{proof}[Вывод]
        Возьмем маленькое изменение угла. Тогда у нас получился почти сектор. $S_{\text{сектор}} = \pi r_{\text{сектор}}^2 \dfrac{\varDelta \varphi}{2 \pi}$. Тогда полная площадь $S = \dfrac{1}{2} \dint_{\varphi_1}^{\varphi_2} r^2(\varphi) d \varphi$.
    \end{proof}
    
    \section{Дифференциальные уравнения}
    
    \begin{definition}[Обыкновенное дифференциальное уравнение]
    	Уравнение вида
    	\[
    ...
    \begin{definition}[Уравнение, разрешённое относительно старшей производной]
    	Если в уравнении $F(x, y(x), y'(x), \dots, y^{(n)}(x)) = 0$ можно выразить старший член, то есть представить его в виде
    	\[
    		y^{(n)}(x) = \tilde{F}(x, y(x), y'(x), \dots, y^{(n-1)}(x)),
    	\]
    	то такое уравнение называется \emph{разрешённым относительно старшей производной}.
    \end{definition}

    \begin{definition}[Ортогональное семейство кривых]
    	Пусть задано семейство кривых
    	\[
    		F(x, y, C) = 0,
    	\]
    	зависящее от параметра $C$. Семейство кривых $G(x, y, C_1) = 0$ называется \emph{ортогональным} к первому, если в каждой точке пересечения касательные к кривым обоих семейств взаимно перпендикулярны.
    \end{definition}

    \begin{method}[Построение ортогонального семейства]
    	Пусть исходному семейству $F(x, y, C) = 0$ соответствует дифференциальное уравнение (не содержащее $C$)
    	\[
    		y' = f(x, y).
    	\]
    	Так как угловые коэффициенты взаимно перпендикулярных прямых удовлетворяют соотношению $k_1 \cdot k_2 = -1$, дифференциальное уравнение ортогонального семейства получается заменой
    	\[
    		y' \; \longmapsto \; -\frac{1}{y'}.
    	\]
    	Общее решение уравнения
    	\[
    		y' = -\frac{1}{f(x, y)}
    	\]
    	и задаёт искомое ортогональное семейство.
    \end{method}

    \begin{example}[Ортогональное семейство для $y = a/x^2$]
    	...
    	\[
    		y' = -\frac{2a}{x^3}.
    	\]
    	Исключим параметр $a$, выразив его из исходного уравнения: $a = yx^2$. Подставляя, получаем дифференциальное уравнение семейства:
    	\[
    		y' = -\frac{2y}{x}.
    	\]
    	Заменяя $y'$ на $-\dfrac{1}{y'}$, получаем уравнение ортогонального семейства:
    	\[
    		y' = \frac{x}{2y}.
    	\]
    	Разделяя переменные и интегрируя:
    	\[
    		2y\,dy = x\,dx \quad\Longrightarrow\quad y^2 = \frac{x^2}{2} + C \quad\Longrightarrow\quad x^2 - 2y^2 = C_1.
    	\]
    	Таким образом, семейство кубических гипербол $y = a/x^2$ и семейство гипербол $x^2 - 2y^2 = C_1$ являются взаимно ортогональными.
    \end{example}
    
        \subsection{Уравнения с разделяющимися переменными}

    \begin{definition}[Уравнение с разделяющимися переменными]
    	Дифференциальное уравнение первого порядка называется уравнением с \emph{разделяющимися переменными}, если оно может быть записано в виде
    	\[
    		f(x)\,dx = g(y)\,dy,
    	\]
    	или, что то же самое,
    	\[
    		y' = \frac{f(x)}{h(y)}, \qquad \text{где } g(y) = h(y).
    	\]
    \end{definition}

    \begin{method}[Решение уравнения с разделяющимися переменными]
    	Пусть уравнение приведено к виду $f(x)\,dx = g(y)\,dy$. Тогда, интегрируя обе части независимо (левую по $x$, правую по $y$), получаем общее решение в неявном виде:
    	\[
    		\int f(x)\,dx = \int g(y)\,dy + C.
    	\]
    	
    	\emph{Замечание.} Если исходное уравнение имеет вид $M(x)N(y)\,dx = P(x)Q(y)\,dy$, то для разделения переменных обе части делят на $N(y)P(x)$ (при условии $N(y)P(x) \ne 0$):
    	\[
    		\frac{M(x)}{P(x)}\,dx = \frac{Q(y)}{N(y)}\,dy.
    	\]
    	При этом отдельно нужно проверить случаи $N(y) = 0$ и $P(x) = 0$ — они могут давать дополнительные (особые) решения, не содержащиеся в общей формуле.
    \end{method}

    \begin{example}
    	Решить уравнение $y' = \dfrac{x}{y}$, $y \ne 0$.
    	...
    	\]
    	Это семейство гипербол, являющееся общим решением исходного уравнения.
    \end{example}

    \subsection{Задача Коши}

    \begin{definition}[Задача Коши]
    	\emph{Задачей Коши} для ОДУ $n$-го порядка
    	\[
    		y^{(n)} = \tilde{F}\bigl(x, y, y', \dots, y^{(n-1)}\bigr)
    	\]
    	называется задача нахождения решения этого уравнения, удовлетворяющего заданным \emph{начальным условиям}
    	\[
    ...
    \begin{note}
    	Для уравнения первого порядка $y' = f(x, y)$ задача Коши состоит в нахождении решения, проходящего через заданную точку $(x_0, y_0)$, то есть удовлетворяющего условию $y(x_0) = y_0$. Геометрически: из всего семейства интегральных кривых (общего решения) выбирается та единственная кривая, которая проходит через точку $(x_0, y_0)$.
    \end{note}

    \begin{theorem}[Коши о существовании и единственности]
    	Пусть функции $F(x, y)$ и $\dfrac{\partial F}{\partial y}(x, y)$ непрерывны в некоторой окрестности точки $(x_0, y_0)$. Тогда в этой окрестности существует единственное решение задачи Коши
    	\[
    		\begin{cases}
    			y' = F(x, y) \\
    			y(x_0) = y_0.
    ...
    \begin{example}
    	...
    	\]
    	Из общего решения (см. пример выше) $y^2 - x^2 = C_1$ подставим начальные условия $x = 0$, $y = 2$:
    	\[
    		2^2 - 0^2 = C_1 \quad\Longrightarrow\quad C_1 = 4.
    	\]
    	Таким образом, частное решение задачи Коши:
    	\[
    		y^2 - x^2 = 4, \qquad \text{т.е.}\qquad y = \sqrt{x^2 + 4}
    	\]
    	(знак $+$ выбран, так как $y(0) = 2 > 0$).
    \end{example}
    
    \begin{statement}
    	\[
			\ln |a| = \ln |b| + C, \ C \in \mathbb{R} \ \Leftrightarrow \ a = C_1 b, \ C_1 \ne 0
		\]
    \end{statement}
\end{document}
