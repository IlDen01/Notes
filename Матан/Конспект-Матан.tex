...
\begin{document}
	...
	\begin{theorem}[Правило Лопиталя.]
		Пусть $f(x)$, $g(x)$ непрерывно дифференцируемы в окрестности точки $x_0$, $x_0 \in \mathbb{R}$. Пусть $\lim\limits_{x \rightarrow x_0} \dfrac{f(x)}{g(x)} = \left[ \dfrac{\infty}{\infty} \right]$ или $\left[ \dfrac{0}{0} \right]$. Тогда, если $\exists \lim\limits_{x \rightarrow x_0} \dfrac{f'(x)}{g'(x)} = a$, то $\lim\limits_{x \rightarrow x_0} \dfrac{f(x)}{g(x)} = a$.
	\end{theorem}
